...
!define MUI_PRODUCT "Project Schranke"
...
;Pages
  ...
  !define MUI_FINISHPAGE_RUN "$INSTDIR\bin\jnglexample.exe"
...
Section ""
  ...
  File ..\build-windows\jnglexample.exe

  CreateShortcut "$desktop\${MUI_PRODUCT}.lnk" "$instdir\bin\jnglexample.exe"
  ...
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MUI_PRODUCT}" \
                   "DisplayIcon" "$INSTDIR\bin\jnglexample.exe"
  WriteRegStr HKCU "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MUI_PRODUCT}" \
